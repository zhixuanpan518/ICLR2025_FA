...
weight1 = [np.concatenate([np.ones(5), -np.ones(5)])]
...
checkpoint_path = 'checkpoint/n=1000_100.pth' 
...
plt.title('min norm/max norm=1/1', fontsize=14)
cbar = plt.colorbar()
...
plt.show()
plt.savefig("figure/Equal_norm.pdf", format='pdf')
...
# im = ax.imshow(matrix, cmap='viridis', interpolation='nearest')  # 使用 'aspect=10' 调整整体比例
# ax.set_title('equal norm', fontsize=14)
...
# plt.yticks(ticks=np.arange(10), labels=mu_labels, fontproperties=font)
# plt.xticks(ticks=np.arange(10), labels=w_labels, fontproperties=font)
# plt.xticks(ticks=np.linspace(0, 90, 10, dtype=int), labels=w_labels[::10], fontproperties=font)
