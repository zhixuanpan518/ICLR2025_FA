...
net.fc = nn.Sequential(
    nn.Linear(num_ftrs, 500),  # 新的全连接层，将特征数从num_ftrs缩小到512
    nn.ReLU(),                 # 添加ReLU激活函数
    # nn.Dropout(0.5),           # 添加Dropout层，防止过拟合
    nn.Linear(500, 2)         # 最后的全连接层，将特征数从512缩小到目标类别数10
)
...
# with torch.no_grad():
#     net.fc[2].weight = nn.Parameter(matrix.t())
#     net.fc[2].bias = nn.Parameter(torch.zeros(2))
...
# for param in net.fc[2].parameters():
#     param.requires_grad = False
...
for i in range(1):
    ...
    #                 loss = F.cross_entropy(logits, y)
    #             grad = torch.autograd.grad(loss, [x])[0]
    #             grad = grad/ torch.norm(grad.view(x.shape[0], -1), p=2, dim=1)
    #             # x = x.detach() + alpha * torch.sign(grad.detach())
    ...
    def test(epoch):
        ...
        with open("twolayer_attack_result/2_test_benign_accuracy.txt", 'a') as file:
            file.write(str(100. * benign_correct / total)+"\n")
        with open("twolayer_attack_result/2_test_adv_accuracy.txt", 'a') as file:
            file.write(str(100. * adv_correct / total)+"\n")
        with open("twolayer_attack_result/2_test_benign_loss.txt", 'a') as file:
            file.write(str(benign_loss)+"\n")
        with open("twolayer_attack_result/2_test_adv_loss.txt", 'a') as file:
    ...
    def train(epoch):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                ...
                adv_outputs = net(adv)
                loss = criterion(adv_outputs, targets)
                # with open("experiment result/super2_train_loss.txt", 'a') as file:
                #     file.write(str(loss.item())+"\n")
                adv_loss += loss.item()
        ...
        with open("twolayer_attack_result/2_train_benign_accuracy.txt", 'a') as file:
            file.write(str(100. * benign_correct / total)+"\n")
        with open("twolayer_attack_result/2_train_adv_accuracy.txt", 'a') as file:
            file.write(str(100. * adv_correct / total)+"\n")
        with open("twolayer_attack_result/2_train_benign_loss.txt", 'a') as file:
            file.write(str(benign_loss)+"\n")
        with open("twolayer_attack_result/2_train_adv_loss.txt", 'a') as file:
            file.write(str(adv_loss)+"\n")
