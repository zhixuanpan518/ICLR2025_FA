...
weight1 = [np.concatenate([np.ones(50), -np.ones(50)])]

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
net = TwoLayerNet(3072, 100, 1).to(device)
...
for epoch in range(200):  
    ...
    if (epoch+1) % 50 == 0:
        checkpoint_path = os.path.join(checkpoint_dir, f'adv_44_{epoch+1}.pth')
        save_checkpoint(net, optimizer, epoch+1, running_loss/len(train_loader), checkpoint_path)
    correct = 0
    predicted = torch.sign(outputs)
    correct += (predicted == labels).sum().item()

    print(f'Accuracy of the model on the training data: {100 * correct / 1000:.2f}%')
